...
**A model nobody approved.** `switchboard drift` extracts every distinct model that answered a request and sets it beside the models the configuration lists. Trivial to describe, almost never run, and it finds things — a model serving production traffic that no review ever passed is a finding no policy document would surface, and the data was already recorded. It states its own blind spot too: the log records the name a caller asked for, not the id it resolved to, so a provider repointed under an unchanged name is invisible to a comparison of names. The policy fingerprint covers the roster including its ids, so the footer counts the fingerprints in force — one means the table is the whole answer, more than one means look. See [docs/drift.md](docs/drift.md).
