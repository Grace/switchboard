...
**A page you can look at.** `switchboard audit view` serves the log read-only over loopback — spend by team, models, redaction counts, recent entries, and the panel a time-series tool cannot show: which policy fingerprint was in force, and where it changed mid-window. Self-contained HTML, no CDN, no JavaScript, no state, so it works on a host with no route to the internet and against a segment downloaded from an archive during an incident. A proof of concept, and it says so on the page.
