...
**It removes the reason you could not buy an observability tool.** In a regulated environment the unresolved compliance question is what stops one being adopted for AI at all: a team wants agents in production, they want real observability for it, and risk asks whether that platform is the system of record for what the models did. The answer is no — it samples, and it keeps sixty days — so the project stalls, or somebody is told to build a bespoke archive first, or a heavyweight governance suite gets bought that does mediocre observability as a side effect. All three are caused by asking one system two questions it was never designed to answer together. switchboard emits OTLP to whatever you already run, Honeycomb included, while keeping the unsampled chained record itself: the observability platform gets to be excellent at observability without pretending to be a system of record, and the compliance question is answered by something built for it. Honeycomb [say as much themselves](https://www.honeycomb.io/blog/infinite-retention-opentelemetry-honeycomb) — *"the needs of observability workloads can sometimes be orthogonal to the needs of compliance workloads"* — and nobody should want that reversed, because a platform keeping everything for seven years would be worse at the job it is for. The span is the audit record minus content: model asked for and model that answered, all four token splits, tools offered, tools refused, the policy fingerprint and the audit id. Neither tier needs an SDK, because the gateway is already in the path, and content never leaves on a span — not by configuration, but because no exported shape has a field able to carry it. See [docs/honeycomb.md](docs/honeycomb.md).
