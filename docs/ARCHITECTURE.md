...
The circuit breaker is per provider. It opens after three transient failures and stays closed to traffic for 15 seconds, after which exactly one request is admitted as a half-open probe. A successful probe closes it immediately; there is no requirement for several successes. A **failed probe re-arms a fresh full 15 seconds**, with no backoff and no decay, so total recovery is 15 seconds multiplied by the number of cycles until a probe succeeds, and has no upper bound while the provider stays unhealthy. This is deliberate — it avoids stampeding a provider that has not recovered — but it means "opens for 15 seconds" describes one cycle, not a ceiling on the outage. Circuit state and token buckets are process-local, not a distributed tenant quota. An exhausted circuit/retry budget returns a visible 503.
